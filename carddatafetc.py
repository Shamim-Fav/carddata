...
def get_gspread_client():
    try:
        ...
        # FIXED: Added all required fields to prevent 'client_id' errors
        creds_dict = {
            ...
            "client_id": "100678312403939380954", # Added this
...
if st.button("🚀 Start Process"):
    ...
    with st.status("Running Scraper...") as status:
        # Phase 1: Fetch
        ...
        # Phase 3: Filtering (THIS ENSURES GOOGLE GETS THE RIGHT DATA)
        ...
        # These are the ONLY columns that will go to Google Sheets
        TARGET_COLS = [
        ...
        # Phase 4: Google Sheets Sync (FILTERED DATA ONLY)
        status.write("📝 Sending Filtered Data to Google Sheets...")
        client = get_gspread_client()
        if client:
            try:
                sh = client.open_by_key(SPREADSHEET_ID)
                ws = sh.get_worksheet(0) # Gets the first tab
                ws.clear()
                
                # Convert only filtered dataframe to list for Google
                ...
                st.success("✅ Google Sheets Updated with Filtered Data!")
            except Exception as e:
    ...
    st.download_button("📥 Download Filtered Excel", buf.getvalue(), f"Cards_{scrape_date}.xlsx")
